...
async function submit() {
  ...
  const { data, error: fetchError } = await useFetch(
    '/prism/openai/v1/chat/completions',
    {
      method: 'POST',
      headers: {
  ...
  <AppLayout :title="title">
    ...
    <div class="h-full flex-col flex">
      <Alert class="mb-4">
        ...
        <AlertDescription>
          This is a demo AI chat. You can use it to test the AI chat.
          And This is subscrition protected page so you can't use it
      ...
      <div v-if="props.subscriptionEnabled" class="flex-1">
        <div class="h-full">
          <div
            class="grid h-full items-stretch gap-6 md:grid-cols-[minmax(0,1fr)_180px]"
          >
            <div class="flex-col space-y-4 flex md:order-2">
              <ModelSelector
                :models="props.models"
                @update:model-select="onModelSelect"
              />
              <TemperatureSelector />
            </div>
            <div class="order-1">
